...
userAccount:function(){var g="";"cadastroCliente"===b.popupType?g="signInPopups":"minhaContaFoto"===b.popupType&&(g="profilePhoto");c.unbind().removeAttr("onclick");var f=c.attr("href")||"";c.bind("click",function(){d.addClass(b.popupClass+" "+g+"Main");""===f&&j&&console.log("N\u00e3o existe URL no atributo href");jQuery('<iframe src="'+f+'" frameborder="0" allowtransparency="true"></iframe>').appendTo(e.addClass(b.popupClass+" "+g));a.show({loading:true});a.positioning();a.close();return false})},
newsletter:function(){c.clone().appendTo(e.addClass(b.popupClass+" newsletterPopup"));d.addClass(b.popupClass+" newsletterMain");a.show();a.positioning();a.close()},quickView:function(){var c=h.find(""!==b.quickViewClass?b.quickViewClass:".quickViewLink"),f=function(){c.filter(":not(.quickViewLinkActivated)").addClass("quickViewLinkActivated").bind("click",function(){jQuery('<iframe src="'+jQuery(this).attr("href")+'" frameborder="0" allowtransparency="true"></iframe>').appendTo(e.addClass(b.popupClass+
" productQuickView"));d.addClass(b.popupClass+" quickViewMain");a.show({loading:true});a.positioning();a.close();return false})};f();i.ajaxStop(f)},paymentForms:function(){var g="",f=function(){var a=h.find(".see-other-payment-method-link");if(a.length<1){j&&console.log("Url das formas de pagamento n\u00e3o encontrado. \n Verifique se o controle esta na p\u00e1gina.\n("+a.selector+")");return false}g=/http:[a-z.\/\?=0-9&]+/i.exec(a[0].onclick.toString())[0]||"#onclickError"};f();c.bind("click",function(){jQuery("<iframe src='"+
g+"' frameborder='0' allowtransparency='true'></iframe>").appendTo(e.addClass(b.popupClass+" paymentFormsPopup"));d.addClass(b.popupClass+" paymentFormsMain");a.show({loading:true});a.positioning();a.close();return false});i.ajaxStop(f)},calculateShipping:function(){i.ajaxStop(function(){var c=h.find("#calculoFrete").children();if(c.length<1)return false;c.find("span.cep-busca a").attr("target","_blank");c.appendTo(e.addClass(b.popupClass+" shippingCalculationPopup"));d.addClass(b.popupClass+" shippingCalculationMain");
a.show();a.positioning();a.close()})},giftList:function(){c.appendTo(e.addClass(b.popupClass+" giftListPopup"));d.addClass(b.popupClass+" giftListMain");a.show();a.positioning();a.close({clickCallback:b.closeCallback})},freeContent:function(){c.appendTo(e.addClass(b.popupClass+" freeContentPopup"));d.addClass(b.popupClass+" freeContentMain");a.show();a.positioning();a.close()},closeNow:function(){a.close({closeNow:true})},giftListReferFriend:function(){var g=function(){var c=$(this).attr("href");
if("undefined"===typeof c||""===c){j&&console.log("[Erro] Url do popup n\u00e3o encontrada.");return false}e.addClass(b.popupClass+" freeContentPopup").load(c);d.addClass(b.popupClass+" giftListReferFriendMain");a.show({loading:true});a.positioning();a.close();return false},f=function(){c.unbind().bind("mouseenter",function(){c.unbind().bind("click",g)})};f();i.ajaxStop(f)}};a.exec();return c};
