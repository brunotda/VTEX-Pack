...
* @date 2012-03-15
*/
jQuery.fn.vtexPopUp2=function(j){var c=jQuery(this);if(1>c.length)return c;var h=jQuery("body"),d=h.find(".boxPopUp2"),k="object"==typeof console;1>d.length&&(d=jQuery('<div class="boxPopUp2"><div class="boxPopUp2-wrap"><span class="boxPopUp2-close"></span><div class="boxPopUp2-content"></div></div></div>'),h.prepend(d),d.after('<div class="boxPopUp2-overlay"></div>'));var b=jQuery.extend({popupType:null,closeContent:null,popupClass:"",initCallback:function(){},closeCallback:function(){}},j),j=d.find(".boxPopUp2-close"),
e=d.find(".boxPopUp2-content"),l=h.find(".boxPopUp2-close, .boxPopUp2-overlay"),m=h.find(".boxPopUp2-overlay"),i=jQuery(document);null!=b.closeContent&&j.html(b.closeContent);var a={positioning:function(){var a=i.scrollTop(),b=jQuery(window).height(),c=d.outerHeight(true);d.css("top",a+(c>=b?20:(b-c)/2)+"px")},show:function(b){b=b||{};m.fadeTo("fast",0.5,function(){d.show().addClass("popupOpened");"boolean"===typeof b.loading&&b.loading===true?a.showLoading():a.hideLoading()})},hideLoading:function(){e.filter(":visible").css("background-image",
"none")},showLoading:function(){e.filter(":visible").css("background-image",'url("/arquivos/ajax-loader.gif")')},close:function(a){var a=a||{},b=function(){m.fadeOut("fast");d.fadeOut("fast",function(){e.empty()});e.attr("class","boxPopUp2-content");d.attr("class","boxPopUp2")};typeof a.closeNow=="boolean"&&a.closeNow==true&&b();if(l.filter(".boxPopUp2-clickActive").length<1){l.addClass("boxPopUp2-clickActive").bind("click",function(){"function"===typeof a.clickCallback&&a.clickCallback();b()});i.bind("keyup",
function(a){(a.keyCode?a.keyCode:a.which)==27&&b()})}},setType:function(){if(c.hasClass("quickViewLink"))a.quickView();else if(c.hasClass("giftListWrap"))a.giftList();else if(c.hasClass("installmentInfoTpl"))a.paymentForms();else if(c.hasClass("shipping-value"))a.calculateShipping();else if(c.hasClass("freeContent"))a.freeContent();else if(c.hasClass("boxPopUp2"))a.closeNow();else if(c.filter("#btnReferAFriend").length>0)a.giftListReferFriend();else if(c.hasClass("lnkAddPhoto")){b.popupType="minhaContaFoto";
a.userAccount()}else return false},checkType:function(){if("cadastroCliente"===b.popupType||"minhaContaFoto"===b.popupType)a.userAccount();else if("newsletter"===b.popupType)a.newsletter();else if("quickview"===b.popupType)a.quickView();else if("giftlist"===b.popupType)a.giftList();else if("paymentforms"===b.popupType)a.paymentForms();else if("shipping"===b.popupType)a.calculateShipping();else if("freecontent"===b.popupType)a.freeContent();else if("closenow"===b.popupType)a.closeNow();else if("GiftListReferAFriend"===
b.popupType)a.giftListReferFriend();else return false},exec:function(){null===b.popupType?a.setType():false===a.checkType()&&a.setType();b.initCallback()},userAccount:function(){var f="";"cadastroCliente"===b.popupType?f="signInPopups":"minhaContaFoto"===b.popupType&&(f="profilePhoto");c.unbind().removeAttr("onclick");var g=c.attr("href")||"";c.bind("click",function(){d.addClass(f+"Main");""===g&&k&&console.log("N\u00e3o existe URL no atributo href");jQuery('<iframe src="'+g+'" frameborder="0" allowtransparency="true"></iframe>').appendTo(e.addClass(b.popupClass+
" "+f));a.show({loading:true});a.positioning();a.close();return false})},newsletter:function(){c.clone().appendTo(e.addClass(b.popupClass+" newsletterPopup"));d.addClass("newsletterMain");a.show();a.positioning();a.close()},quickView:function(){i.ajaxStop(function(){c.filter(":not(.quickViewLinkActivated)").addClass("quickViewLinkActivated").bind("click",function(){jQuery('<iframe src="'+jQuery(this).attr("href")+'" frameborder="0" allowtransparency="true"></iframe>').appendTo(e.addClass(b.popupClass+
" productQuickView"));d.addClass("quickViewMain");a.show({loading:true});a.positioning();a.close();return false})})},paymentForms:function(){var f="",g=function(){var a=h.find(".see-other-payment-method-link");if(a.length<1){k&&console.log("Url das formas de pagamento n\u00e3o encontrado. \n Verifique se o controle esta na p\u00e1gina.\n("+a.selector+")");return false}f=(a[0].onclick||"#onclickError").toString().split('"')[1]||"#onclickError"};g();c.bind("click",function(){jQuery("<iframe src='"+
f+"' frameborder='0' allowtransparency='true'></iframe>").appendTo(e.addClass(b.popupClass+" paymentFormsPopup"));d.addClass("paymentFormsMain");a.show({loading:true});a.positioning();a.close();return false});i.ajaxStop(g)},calculateShipping:function(){i.ajaxStop(function(){var c=h.find("#calculoFrete").children();if(c.length<1)return false;c.find("span.cep-busca a").attr("target","_blank");c.appendTo(e.addClass(b.popupClass+" shippingCalculationPopup"));d.addClass("shippingCalculationMain");a.show();
...
console.log("[Erro] Url do popup n\u00e3o encontrada.");return false}e.addClass(b.popupClass+" freeContentPopup").load(c);d.addClass("giftListReferFriendMain");a.show({loading:true});a.positioning();a.close();return false},g=function(){c.unbind().bind("mouseenter",function(){c.unbind().bind("click",f)})};g();i.ajaxStop(g)}};a.exec();return c};
