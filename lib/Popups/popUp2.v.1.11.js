	...
	var fns=
    {
        ...
        close:function(options)
        {
			...
			if($this.hasClass("autoClose"))
			{
				...
				if(isNaN(parseInt(seconds))){if(_console) console.log("[Erro] O tempo informado (em segundos) não é um valor numérico: “"+seconds+"”"); return false;}
		...
		userAccount:function()
		{
			...
			$this.bind("click",function(e){
				popUpElem.addClass(newClass+"Main");
		...
		newsletter:function()
		{
			...
			popUpElem.addClass("newsletterMain");
			fns.show();
			fns.positioning();
			fns.close();
		},
		...
		{
			var fn=function(){
				$this.filter(":not(.quickViewLinkActivated)").addClass("quickViewLinkActivated").bind("click",function(){
					...
					popUpElem.addClass("quickViewMain");
					fns.show({"loading":true});
					fns.positioning();
					fns.close();
					return false;
		...
		paymentForms:function()
		{
			...
			var setLink=function()
			{
				...
				url=(link[0].onclick||"#onclickError").toString().split('"')[1]||"#onclickError";
			};
			setLink();
			...
			$this.bind("click",function(){
				...
				popUpElem.addClass("paymentFormsMain");
				fns.show({"loading":true});
				fns.positioning();
				fns.close();
				return false;
		...
		calculateShipping:function()
		{
			var fn=function(){
				...
				popUpElem.addClass("shippingCalculationMain");
				fns.show();
				fns.positioning();
				fns.close();
			};
		...
		giftList:function()
		{
			...
			popUpElem.addClass("giftListMain");
			fns.show();
			fns.positioning();
		...
		freeContent:function()
		{
			...
			popUpElem.addClass("freeContentMain");
			fns.show();
			fns.positioning();
			fns.close();
		},
		...
		giftListReferFriend:function()
		{
			var func=function(e){
				...
				popUpElem.addClass("giftListReferFriendMain");
				fns.show({"loading":true});
				fns.positioning();
				fns.close();
				return false;
