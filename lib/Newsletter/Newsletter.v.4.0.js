...
(function($){$.fn.vtexNews=$.fn.vtexNews2=function(opts){
	var _this,defaults,options,animateField,log,extTitle;;
	...
	defaults={
		...
		animation:null, // Escolha o método de animação: "leftRight","blink"
	...
	console.log(options.validationMethod);
	
	// Checando se o plugin de popUp2 esta na página
	if(options.validationMethod=="popup" && "function"!==typeof jQuery.fn.vtexPopUp2){log("O popUp2 não foi encontrado. Adicione o Plugin de PopUp2."); return _this;}
	...
	_this.each(function(){
		...
		// Reportando erros
		if(nameField.length<1 && options.checkNameExist){log("Campo de nome, não encontrado ("+nameField.selector+"). Será atribuido um valor padrão.","info");}
		if(emailField.length<1){log("Campo de e-mail, não encontrado ("+emailField.selector+")"); return elem;}
		if(btn.length<1){log("Botão de envio, não encontrado ("+btn.selector+")"); return elem;}
		if(options.validationMethod!="animateField" && (elemSuccess.length<1 || elemError.length<1)){log("A(s) mensagem(ns) de erro e/ou sucesso esta(m) faltando \n ("+elemSuccess.selector+", "+elemError.selector+")"); return elem;}
